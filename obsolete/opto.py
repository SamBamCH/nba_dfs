    ...
    def load_projections(self, path):
        ...
        with open(path, encoding="utf-8-sig") as file:
            ...
            for row in reader:
                ...
                self.secondary_projection_minimum = 10

                if team not in self.teams_in_player_ids:
                    continue

                    # Check if player meets the secondary projection minimum but not the main minimum
                if self.secondary_projection_minimum <= fpts < self.projection_minimum:
                    print(
                        f"Excluded due to projection: {player_name}, {position}, {team}, Minutes: {minutes}, Pts/$: {pts_per_dollar}, Fpts: {fpts}")

                    # Check if player meets the projection minimum
                if fpts < self.projection_minimum:
                    continue

                    # Check if player meets the minimum minutes
                if minutes < self.min_minutes:
                    if fpts >= self.secondary_projection_minimum:
                        print(
                            f"Excluded due to insufficient minutes: {player_name}, {position}, {team}, Minutes: {minutes}, Pts/$: {pts_per_dollar}, Fpts: {fpts}")
                    continue
    ...
    def print_top_150_lineup_exposures(self):
        ...
        for player, count in player_selections.items():
            exposure_record = {
                ...
                'Projected Value': self.player_dict[player]["Value"],
                'Ownership': self.player_dict[player]["Ownership"]
            }
            exposures_data.append(exposure_record)
